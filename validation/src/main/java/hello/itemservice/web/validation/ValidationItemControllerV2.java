...
public class ValidationItemControllerV2 {
    ...
    @InitBinder
    public void init(WebDataBinder dataBinder) { // 컨트롤러가 호출될때 aop 방식으로 validator 를 호출 가능
        ...
        if (item.getPrice() != null && item.getQuantity() != null) {
            ...
            if (resultPrice < 10000) {
                bindingResult.addError(
                        new ObjectError(
                                "item",
                                "가격* 수량의 합은 10000원 이상이여야 합니다. 현재 값 = " + resultPrice)
                );
            }
        }

        // 검증에 실패하면 다시 입력 화면으로, bindingResult 는 스프링이 알아서 Model 에 넣음
    ...
    public String addItemV2(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        if (item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1000000) {
            bindingResult.addError(new FieldError("item", "price", item.getPrice(),
                    false, null, null, "가격은 1000 ~ 1000000 까지 허용합니다"));
        }
        if (item.getQuantity() == null || item.getQuantity() >= 9999) {
            bindingResult.addError(new FieldError("item", "quantity", item.getQuantity(),
                    false, null, null, "수량은 최대 9999개 까지 허용합니다"));
        }
...
//    @PostMapping("/add")
    public String addItemV4(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        // 검증 로직을 지원하는 검증유틸리티 클래스도 있음 , 단순한 기능만 지원
...
//    @PostMapping("/add")
    ...
    @PostMapping("/add") // @Validated 어노테이션으로 item 객체를 aop 검증 수행
